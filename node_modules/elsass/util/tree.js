...
class Tree {
  ...
  appendChildrenAtoms (parentAtomId, parentId, depth, data) {
    ...
    if (_this.atoms[parentAtomId] && data && (!_this.max_depth || _this.max_depth > depth)) {
      for (let v of data) {
        const type = v.params_key ? v.params_key : v.name
        const name = v.key ? v.key : v.name
        let _atom = _this.__appendAtom(type, name, depth + 1, parentAtomId, parentId, v.value, v)
        _groups.push(_atom.id)
        // 给双亲添加子节点
        _this.atoms[parentAtomId].children.push(_atom.id)
        // _this.atoms[parentAtomId].children_atoms.push(_atom)
  ...
  // 添加节点方法
  __appendAtom (type, name, depth, parentAtomId, parentId, rId, options = {}, status = 0, expanded = 0, display = 1) {
    let _this = this
    let atom = new Atom({
  ...
  setAtomsStatus (data, status, type = null) {
    ...
    for (let i of data) {
      for (let j in _this.atoms) {
        if (i.value === _this.atoms[j].r_id && _this.atoms[j].type === (type || i.params_key)) {
  ...
  setAtomDisplay (value, _atom) {
    ...
    if (value === 1) {
      _atom.ancestors.forEach((item, index) => {
        this.atoms[item].display = value
        this.atoms[item].expanded = value
      })
    } else {
      _atom.display = 0
    }
  ...
  setChildrenStatusRecursive (id, status) {
    let _this = this
    if (_this.atoms[id] && _this.atoms[id].children && _this.atoms[id].children.length > 0) {
      for (let i of _this.atoms[id].children) {
  ...
  setStatusWithParent (id, status) {
    ...
    let statusCount = 0
    if (_this.atoms[paretnAtomId] && _this.atoms[paretnAtomId].children) {
      for (let i of _this.atoms[paretnAtomId].children) {
        if (_this.atoms[i].status === status) {
          statusCount++
        }
      }
    }
    if (statusCount !== 0 && statusCount === _this.atoms[paretnAtomId].children.length) {
  ...
  childrenStatusIsSame (id, status) {
    ...
    let _this = this
    let _atom = this.atoms[id]
    for (let i of _atom.children) {
      if (_this.atoms[i].status === status) {
        count++
      }
    }
    return count === _atom.children.length
  }

  // 设置祖先节点状态
  setAncestorsStatus (id, status) {
    let _this = this
    if (_this.atoms[id] && _this.atoms[id].ancestors && _this.atoms[id].ancestors.length > 0) {
      for (let i of _this.atoms[id].ancestors) {
        _this.setAtomStatus(i, status, false)
      }
    }
  }
  ...
  searchAtomByLabel (label, depth) {
    ...
    // 搜索某一层级
    let search_reg = new RegExp(label, 'g')
    ...
    for(let item of this.atoms) {
      ...
      if ((depth && depth !== item.depth) || !item.label.match(label)) {
        ...
        search_result_count++
        item.show_label = item.label.replace(search_reg, matched)
